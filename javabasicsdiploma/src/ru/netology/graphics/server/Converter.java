...
public class Converter implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        int pictureWidth = img.getWidth ();
        int pictureHeight = img.getHeight ();
        int scaleFactor;
        ...
        if (pictureRatio > maxRatio) {
            throw new BadImageSizeException (pictureRatio, maxRatio);
        }   else if (pictureWidth > pictureHeight) {
            scaleFactor = pictureWidth / width;
        } else {
            scaleFactor = pictureHeight / height;
        }

        int newWidth = pictureWidth / scaleFactor;
        int newHeight = pictureHeight / scaleFactor;
        ...
        String[][] cArray = new String[bwImg.getWidth ()][bwImg.getHeight ()];

        for (int w=0; w < bwImg.getWidth (); w++){
            for (int h=0; h < bwImg.getHeight (); h++){
                int color = bwRaster.getPixel (w, h, new int[3])[0];
                char c = schema.convert (color);
                cArray [w][h] = String.valueOf (c); //запоминаем символ c, например, в двумерном массиве или как-то ещё на ваше усмотрение
            }
        }
